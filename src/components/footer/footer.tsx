...
function FooterContent({ label, url }: FooterContentProps) {
    return (
        <a href={url} target="_blank" rel="noopener noreferrer" className={`${splineSansMono.className} text-xs font-light text-[var(--color-text-default)] hover:opacity-80 transition-opacity`}>
            <div className="flex flex-row items-center justify-center">
                {label}
                <ArrowUpRight className="w-4 h-4" color="var(--color-text-default)" />
            </div>
        </a>
    )
}
